...
//             if (typeof img.naturalWidth == 'number' && typeof img.naturalHeight == 'number') complete = !(this.naturalWidth == 0 && this.naturalHeight == 0); else if (typeof img.complete == 'boolean') complete = img.complete;
//         });
//         return complete;
//     };
...
//         var n = this, c = c == undefined ? 2 : (isNaN(c = Math.abs(c)) ? 2 : c), d = d == undefined ? "." : d,
...
//         }
//         if (!chg) {
//             if (classes.length == 1 && classes[0] == '') classes[0] = icon; else classes.push(icon);
//         }
//         return classes.join(' ');
//     };
...
//         var msg = jQuery('.trx_addons_message_box_system'), msg_delay = 5000;
//         if (msg.length > 0) {
//             setTimeout(function () {
...
//                     if (obj.length > 0) {
//                         var off = obj.offset().top, scroll = jQuery(window).scrollTop(),
...
//         if ( !container.hasClass('animate_to_inited')) {
...
//             });
//         }
//         var cp = container.find('.trx_addons_color_selector:not(.inited)'), cp_created = false;
//         if (cp.length > 0) {
...
//             var clr = fld.val(), hsb = trx_addons_hex2hsb(clr);
...
//                 if (linked_field === undefined) linked_field = range_slider.prev('input[type="hidden"]'); else linked_field = jQuery('#' + linked_field);
...
//                     }
//                 };
//                 if (range_slider_type === true) init_obj.values = values; else init_obj.value = values[0];
...
//         //                         if (openerElement.parents('.trx_addons_hover').find('img').length > 0) openerElement = openerElement.parents('.trx_addons_hover').find('img'); else if (openerElement.find('img').length > 0) openerElement = openerElement.find('img'); else if (openerElement.siblings('img').length > 0) openerElement = openerElement.siblings('img'); else if (openerElement.parent().parent().find('img').length > 0) openerElement = openerElement.parent().parent().find('img');
//         //                     }
...
//         //         callbacks: {
//         //             open: function () {
//         //                 if (jQuery(this.content).data('popup-content') === undefined) jQuery(this.content).data('popup-content', jQuery(this.content).html()); else jQuery(this.content).html(jQuery(this.content).data('popup-content'));
...
//         //                 jQuery(this.content).find('.video_frame > iframe, iframe').each(function () {
...
//             container.find('a.post_counters_likes:not(.inited),a.comment_counters_likes:not(.inited)').addClass('inited').on('click', function (e) {
//                 if (!likes_busy) {
//                     likes_busy = true;
...
//                         }
//                         if (rez.error === '') {
//                             var cookie_likes = trx_addons_get_cookie('trx_addons_emotions'), cookie_likes_new = ',';
//                             if (cookie_likes) {
...
//             var col = jQuery(this), row = col.parent();
...
//                     if (scroll_dir == 0) scroll_dir = TRX_ADDONS_STORAGE['scroll_dir_last']; else scroll_dir = scroll_dir > 0 ? 1 : -1;
...
//         var recursive = arguments[1] !== undefined ? arguments[1] : 1;
//         var showMethods = arguments[2] !== undefined ? arguments[2] : false;
//         var level = arguments[3] !== undefined ? arguments[3] : 0;
//         var dispStr = "";
//         var addStr = "";
//         var curStr = "";
//         if (level > 0) {
...
//                 if (level < recursive && (typeof(obj[prop]) == 'object' || typeof(obj[prop]) == 'array') && obj[prop] != obj) dispStr += addStr + prop + '=' + trx_addons_debug_object(obj[prop], recursive, showMethods, level + 1); else {
//                     try {
//                         curStr = "" + obj[prop];
//                     } catch (e) {
//                         curStr = "--- Not evaluate ---";
...
//         console.log(s);
//     });
//     window.dco === undefined && (window.dco = function (s, r, m) {
//         console.log(trx_addons_debug_object(s, r, m));
//     });
//     window.dal === undefined && (window.dal = function (s) {
//         if (TRX_ADDONS_STORAGE['user_logged_in']) alert(s);
//     });
//     window.dao === undefined && (window.dao = function (s, r, m) {
//         if (TRX_ADDONS_STORAGE['user_logged_in']) alert(trx_addons_debug_object(s, r, m));
//     });
//     window.ddl === undefined && (window.ddl = function (s, c) {
//         trx_addons_debug_log(s);
//     });
//     window.ddo === undefined && (window.ddo = function (s, r, m, c) {
//         trx_addons_debug_log(trx_addons_debug_object(s, r, m), c);
//     });
// })();
// jQuery(document).on('action.ready_trx_addons', function () {
//     "use strict";
...
//                 max_length: {value: 60, message: TRX_ADDONS_STORAGE['msg_password_long']}
//             }, {field: "pwd2", equal_to: {value: 'pwd', message: TRX_ADDONS_STORAGE['msg_password_not_equal']}}]
//         });
//         if (!error) {
//             jQuery.post(TRX_ADDONS_STORAGE['ajax_url'], {
...
//         jQuery('.sc_item_pagination:not(.inited)').addClass('inited').on('click', 'a', function (e) {
//             if (!pagination_busy) {
//                 pagination_busy = true;
//                 var link = jQuery(this), pagination_wrap = link.parents('.sc_item_pagination'),
//                     sc = pagination_wrap.parent(), posts = pagination_wrap.prev(),
...
//                     if (rez.error === '') {
//                         if (rez.css != '') {
//                             var selector = 'trx_addons-inline-styles-inline-css', inline_css = jQuery('#' + selector);
//                             if (inline_css.length == 0) jQuery('body').append('<style id="' + selector + '" type="text/css">' + rez.css + '</style>'); else inline_css.append(rez.css);
//                         }
//                         if (load_more) {
...
//                                     if (idx > 0) {
//                                         var classes = jQuery(this).attr('class').split(' '), found = false;
//                                         for (var i = 0; i < classes.length; i++) {
...
//                                     }
//                                 });
//                             } else posts.append(jQuery(rez.data).find('.sc_item_posts_container').html());
//                             jQuery(document).trigger('action.init_hidden_elements', [posts]);
//                             if (link.data('page') >= link.data('max-page')) pagination_wrap.fadeOut(); else link.data('page', Number(link.data('page')) + 1);
//                         } else {
//                             sc.fadeOut(function () {
...
// jQuery(document).on('action.ready_trx_addons', function () {
//     "use strict";
//     var rows = jQuery('.sc_layouts_row_fixed'), rows_always = jQuery('.sc_layouts_row_fixed_always');
//     if (rows.length > 0) {
//         rows.each(function () {
...
//                 }
//             });
//             if (rows_always.length == 0) return; else rows = rows_always;
//         }
//         var scroll_offset = jQuery(window).scrollTop();
...
//     } else if (toc_menu.hasClass('inited')) return;
...
//                 doit = true;
//                 setTimeout(function () {
//                     if (items.prev >= 0 && items.prevOffset >= scroll_offset - wh - ah) trx_addons_scroll_to_anchor(toc_menu_items.eq(items.prev).find('a'), false); else trx_addons_document_animate_to(Math.max(0, scroll_offset - wh));
//                 }, 10);
//             } else {
//                 doit = true;
//                 setTimeout(function () {
//                     if (items.next >= 0 && items.nextOffset <= scroll_offset + wh + ah) trx_addons_scroll_to_anchor(toc_menu_items.eq(items.next).find('a'), false); else trx_addons_document_animate_to(Math.min(jQuery(document).height(), scroll_offset + wh));
//                 }, 10);
//             }
//             if (doit) {
//                 wheel_busy = true;
...
//         var items = {loc: '', current: [], prev: -1, prevOffset: -1, next: -1, nextOffset: -1};
...
//         if (field.val() != '') field.addClass('filled'); else field.removeClass('filled');
//     }
//
...
// (function () {
//     "use strict";
//     var googlemap = {'inited': false, 'count': 0, 'geocoder': null, 'maps': []};
//     var timer = null;
//     jQuery(document).on('action.init_hidden_elements', function (e, container) {
//         if (container === undefined) container = jQuery('body');
...
//         if (googlemap.maps[id].opt['zoom'] < 1) googlemap.maps[id].bounds = new google.maps.LatLngBounds(); else googlemap.maps[id].map.setZoom(googlemap.maps[id].opt['zoom']);
...
//                             if (results[0].geometry.location.lat && results[0].geometry.location.lng) googlemap.maps[id].markers[idx].latlng = '' + results[0].geometry.location.lat() + ',' + results[0].geometry.location.lng(); else googlemap.maps[id].markers[idx].latlng = results[0].geometry.location.toString().replace(/\(\)/g, '');
...
//                     googlemap.maps[id].markers[i].infowindow = new google.maps.InfoWindow({content: googlemap.maps[id].markers[i].description});
...
//                 maxZoom: 18,
//                 gridSize: 60,
//                 styles: [{url: googlemap.maps[id].clusterIcon, width: 48, height: 48, textColor: "#fff"}]
//             });
...
//     if (this.div_) {
//         var a = "", b = this.backgroundPosition_.split(" "), c = parseInt(b[0].trim(), 10),
//             d = parseInt(b[1].trim(), 10), e = this.getPosFromLatLng_(this.center_);
...
//             var e = this.markers_.length + 1, f = (this.center_.lat() * (e - 1) + a.getPosition().lat()) / e,
...
//         var newElement = {}, x = parseFloat(element.x) || 0, y = parseFloat(element.y) || 0,
//             width = parseFloat(element.width) || 0, height = parseFloat(element.height) || 0;
//         newElement.d = 'M' + x + ' ' + y + ' ';
...
//         var base = -Math.cos(x * (0.5 * Math.PI)) + 1, rate = Math.pow(base, 1.5), rateR = Math.pow(1 - x, 2),
...
//         var scrolled = this.scrollY(), viewed = scrolled + this.getViewportH(), elBCR = el.getBoundingClientRect(),
//             elHeight = elBCR.height, elTop = scrolled + elBCR.top, elBottom = elTop + elHeight;
//         h = h || 0;
...
// jQuery(document).on('action.ready_trx_addons', function () {
//     "use strict";
//     jQuery('.sc_recent_news_header_category_item_more:not(.inited)').addClass('inited').on('click', function () {
...
//                     if (jQuery(this).hasClass('slider_next')) TRX_ADDONS_STORAGE['swipers'][slider_id].slideNext(); else TRX_ADDONS_STORAGE['swipers'][slider_id].slidePrev();
//                     e.preventDefault();
//                     return false;
//                 });
...
//                 }
//                 if (on_resize && slider.hasClass('slider_swiper') && typeof TRX_ADDONS_STORAGE['swipers'][id].onResize !== 'undefined') TRX_ADDONS_STORAGE['swipers'][id].onResize();
//             }
//         });
//     }
//
...
//                     if (in_group == 1) in_group = 2; else w_siblings = 0;
//                 } else {
//                     if (jQuery(this).find('#' + ul_id).length > 0) in_group = 1; else w_siblings += (jQuery(this).outerWidth() + Math.ceil(parseFloat(jQuery(this).css('marginLeft'))) + Math.ceil(parseFloat(jQuery(this).css('marginRight'))));
//                 }
//             });
//             w_max -= w_siblings;
//             var w_all = 0;
...
//         options = $.extend({overlap: 0, style: 'box', reset: 50, color: '#00c6ff', colorOverride: true,}, options);
//         return this.each(function () {
//             var nav = $(this),
//                 currentPageItem = nav.find('>.current-menu-item,>.current-menu-parent,>.current-menu-ancestor'),
//                 hidden = false, blob, reset;
...
//             }).appendTo(this);
//             blob = $('#blob', nav);
//             if (options.style == 'box') blob.css({height: currentPageItem.outerHeight() + options.overlap});
...
//                     if (search_wrap.hasClass('search_opened')) trx_addons_search_close(search_wrap); else search_field.get(0).focus();
//                 } else {
