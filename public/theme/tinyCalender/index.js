...
function TinyPicker(overrides) { // eslint-disable-line no-unused-vars
    ...
    var today = newDateInstance(newDateInstance().setHours(0, 0, 0, 0)+1);
    var wroteCss = false;
    ...
    function setDateInEl(date, shadowElement, initial) {
        ...
        if (date instanceof Date && shadowElement instanceof HTMLElement) {
            let formattedDate = date.toLocaleDateString(settings.local, settings.localOpts);
            shadowElement.value = formattedDate;
            shadowElement.innerHTML = formattedDate;
            shadowElement.setAttribute('date', getTime(date));

            // Ensure the calendar closes only after the date is set
            if (!initial) {
                setTimeout(() => {
                    handleCalendarState(shadowElement, date);
                }, 100); // Delay closing to ensure input updates
            }
        }
    }
    ...
    this.init = function () {
        [firstBox, lastBox].forEach(function (element) {
            ...
            // ✅ Ensure selection updates input field before closing calendar
            addEventListener(element, 'change', function () {
                setDateInEl(newDateInstance(element.value), element, false);
            });
        });
    };
