...
<body onload="payzonePaymentPageLoad();">
  <form class='payzone-form' id='payzone-payment-form' name='payzone-payment-form' target="_self" method="POST" style='display:none' action="<?php echo $PayzoneGateway->getURL('form-action-payment'); ?>" >
    <?php
    if ($PayzoneGateway->getIntegrationType() == \Payzone\Constants\INTEGRATION_TYPE::DIRECT){ ?>
      ...
      if ($PayzoneGateway->getPayzoneImages()) {
        ...
        if ($PayzoneGateway->getIntegrationType() == \Payzone\Constants\INTEGRATION_TYPE::DIRECT) {
          require_once(__DIR__ . "/includes/templates/card-details.php");
        }
      ?>
  ...
  echo $PayzoneGateway->buildFormRequest(); ?>
  </form>
  <?php
